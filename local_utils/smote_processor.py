...
def apply_smote_to_benign(
    ...
    if class_counts_before[0] < target_class_size:
        ...
        try:
            ...
            # Create new DMatrix with the resampled data
            train_dmatrix_resampled = xgb.DMatrix(X_resampled, label=y_resampled, missing=np.nan)
            
            return train_dmatrix_resampled, valid_dmatrix, processor
            
        except Exception as e:
            log(INFO, f"Error applying SMOTE: {str(e)}")
